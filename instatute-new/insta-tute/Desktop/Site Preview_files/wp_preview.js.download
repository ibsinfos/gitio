...
function showLoadingDots() {
  ...
  if(loading.length == 0) {
    ...
    loading.css("background", "url(window.rconfWar + '/_dm/s/rt/images/nav-loader.gif') no-repeat center 40px  white");
...
function removeCssAttributes(selector, attributesArray, ruleValidator, stylesheet) {
  ...
  removeCssAttributesFromStyleSheet(selector, attributesArray, ruleValidator,
    styleSheet);
...
function refreshGlobalAndThemeCssInternal(themeCss, globalCss, timeout) {
  ...
  if(newStyle.styleSheet)
    newStyle.styleSheet.cssText = globalCss;
  //	    },0);
...
function refreshPageStyleCss(css, timeout, isHeader) {
  isHeader = isHeader || false;
  var newStyle = document.createElement("style");

  jQuery(newStyle).html(css);
  jQuery(newStyle).attr("id", isHeader ? "newheaderCss" : "newpagestyle");
  jQuery('head').append(newStyle);
  if(timeout == null) {
    timeout = 10;
  }
  if(timeout == 0) {
    removeOldPageStyle(isHeader);
    initComponents();
  } else {
    setTimeout("removeOldPageStyle(" + isHeader + "); initComponents();", timeout);
  }
}

function refreshPageStyleDeviceCss(css, timeout, isHeader) {
  isHeader = isHeader || false;
  var newStyle = document.createElement("style");

  jQuery(newStyle).html(css);
  jQuery(newStyle).attr("id", isHeader ? "newheaderDeviceCss" : "newpagestyleDevice");
  jQuery('head').append(newStyle);
  if(timeout == null) {
    timeout = 10;
  }
  if(timeout == 0) {
    removeOldDevicePageStyle(isHeader);
    initComponents();
  } else {
    setTimeout("removeOldDevicePageStyle(" + isHeader + "); initComponents();", timeout);
  }
}
...
_findCurrentNavText = function() {
  ...
  if(hiddenNavElement.size() > 0) {
    ...
    if(currentLI.size() > 0) {
      return $(currentLI.get(0)).find(".navText:first").text();
    }
  }
  return "";
};
...
function removeStyleAndReplace(oldStyleId, newStyleId) {
  ...
  oldStyle.parentNode.insertBefore(newStyle, oldStyle.nextSibling);
...
function removeOldPageStyle(isHeader) {
  ...
  //	var styleId = isHeader?"headerCss":"pagestyle";
  //	var oldStyle = document.getElementById(styleId);
  //	oldStyle.parentNode.removeChild(oldStyle);
  //	var newStyle = document.getElementById(isHeader?"newheaderCss":"newpagestyle");
  //	newStyle.setAttribute("id", styleId);
  //	hideLoading();
  //
  //	var tempStyle = document.getElementById("tempCss");
...
function removeOldDevicePageStyle(isHeader) {
  ...
  removeStyleAndReplace(isHeader ? "headerDeviceCss" : "pagestyleDevice", isHeader ? "newheaderDeviceCss" : "newpagestyleDevice");

  //	var styleId = isHeader?"headerDeviceCss":"pagestyleDevice";
  //	var oldStyle = document.getElementById(styleId);
  //	oldStyle.parentNode.removeChild(oldStyle);
  //	var newStyle = document.getElementById(isHeader?"newheaderDeviceCss":"newpagestyleDevice");
  //	newStyle.setAttribute("id", styleId);
  //	hideLoading();
  //	
  //	var tempStyle = document.getElementById("tempCss");
...
function loadStoreScript(storeId) {
  ...
  script.src = "https://app.multiscreenstore.com/script.js?" + storeId;
