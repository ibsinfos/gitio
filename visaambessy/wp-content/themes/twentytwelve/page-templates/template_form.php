...
$Email=$_POST['Email'];
$typekids=$_POST['typekids'];
  $day=$_POST['date_day'] ;

 $month=$_POST['date_month'] ;
 $yesr=$_POST['date_year'] ;
//echo $day . '-' . $month . '-' . $yesr;
  $age = array($day,$month,$yesr);
 $age = implode("-",$age);
  /*echo  $age2;
   die;*/
...
$wish_to_migrate= implode(',', $_POST['wish_to_migrate']);
...
$stillemploye=$_POST['stillemploye'];
$postal=$_POST['postal'];
$experiencemonth=$_POST['experiencemonth'];
$aplydate=date("Y-m-d"); 
global $wpdb;
  
  $sql="insert into enquiries(whyvisa,visatype,title,firstname,lastname,nationality,dob_day,dob_month,dob_year,email,contactnumber,cellnumber,address1,address2,destinationcountry,residingcountry,
  require_time,occupation,education,exprienceduration,AssistanceWithVisa,currentjobtitle,StillInthisEmployment,employdurationLast2,employduration,postalcode,applydate) values('$typekids','$Type_of_Visa','$title','$Condidate','$lastname','$Nationality','$day','$month',
'$yesr','$Email','$Contact','$telephone','$address','$comments','$wish_to_migrate','$current_live','$planning','$job_title','$Qualifications','$experience','$assistance','$currentjob','$stillemploye','$experiencemonth','$experienceyear','$postal','$aplydate')";
...
//mysql_query("insert into enquiries values('','$title','$Condidate','$lastname','$telephone','$Email','$age','$Contact','$address','$wish_to_migrate','$current_live',
//'$planning','$Nationality','$Type_of_Visa','$job_title','$Qualifications','$experience','$assistance','$file','$comments')")or die(mysql_error());

//mysql_query("insert into visaform values('','$title','$Condidate','$lastname','$telephone','$Email','$age','$Contact','$address','$wish_to_migrate','$current_live',
//'$plann`````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````ing','$Nationality','$Type_of_Visa','$job_title','$Qualifications','$experience','$assistance','$file','$comments')")or die(mysql_error()); 
*/
$location = get_template_directory() . '/resume/' . $file;
