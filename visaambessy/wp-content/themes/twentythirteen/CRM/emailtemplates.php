...
<script type="text/javascript" src="http://global-migrate.com/CRM/ckeditor/ckeditor.js"></script>
	<script src="http://global-migrate.com/CRM/ckeditor/_samples/sample.js" type="text/javascript"></script>
	<link href="http://global-migrate.com/CRM/ckeditor/_samples/sample.css" rel="stylesheet" type="text/css" />
</head>
<?php

if(isset($_REQUEST["btnSend"]))
{
		//echo($_FILES["txtName"]["name"]);
		$uploads_dir = 'attachfiles';
		$tmp_name = $_FILES["txtName"]["tmp_name"];
		$name = $_FILES["txtName"]["name"];
						
		if (!move_uploaded_file($tmp_name, "$uploads_dir/$name")) {
			
			
		} else {
		$sql="insert into emailtemplates (name) values('$name')";
		//	echo($sql);
	...
	if(isset($_REQUEST['task']) && $_REQUEST['task']!="delete")
	{
		...
		$sql="delete from emailtemplates where id_temp='$_REQUEST[id]'";
		$record=$conn->execute($sql);		
	}
?>
<body bgcolor="#cccccc">
 <fieldset>
  <legend>Email Tempalte</legend>
  <table width="700" border="0" cellspacing="4" cellpadding="4" bgcolor="#CCCCCC">
  <tr>
    <td valign="top" bgcolor="#CCCCCC"><? include_once("includes/topmenu.php");?></td>
  </tr>
    <form action="" method="post" enctype="multipart/form-data">
   <input type="hidden" name="templateid" value="<?php echo($record->fields['id_temp']); ?>" />
 
    <tr>
      <td>&nbsp;</td>
      <td>Template File</td>
      <td> <input name="txtName" type="file"  class="tasksfield" id="txtName" /> </td>
      <td>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      ...
      <td>
        ...
        <input name="btnSend" type="submit" id="Send" value="Send">     </td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
  ...
  <table width="100%" border="0" class="maintable" align="center" height="100%">

  
  <tr>
    <td height="250" valign="top">
	<? include_once("includes/emailtemplates_grid.php"); ?>	
	</td>
  </tr>
