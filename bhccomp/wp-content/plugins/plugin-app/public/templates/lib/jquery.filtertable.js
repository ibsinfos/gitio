...
(function($) {
    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        var defaults = { // start off with some default settings
                ...
                inputSelector:     '#zoeken',                // use the element with this selector for the filter input field instead of creating one
                ...
                placeholder:       'Zoek client:', // HTML5 placeholder text for the filter field
        ...
        var doFiltering = function(table, q) { // handle the actual table filtering
                ...
                if (q==='') { // if the filtering query is blank
                    ...
                    tbody.find('li').hide().removeClass(settings.visibleClass); // hide all rows, assuming none were found
                    if (settings.hideTFootOnFilter) { // hide footer if the setting was specified
                        table.find('tfoot').hide();
                    }
                    tbody.find('li').removeClass(settings.highlightClass).filter(':filterTableFind("'+q.replace(/(['"])/g,'\\$1')+'")').addClass(settings.highlightClass).closest('li').show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
                }
