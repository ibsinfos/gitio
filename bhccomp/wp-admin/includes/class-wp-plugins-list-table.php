...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function single_row( $item ) {
		...
		if ( 'mustuse' === $context ) {
			...
			$plugin_app = $plugin_file;
			if ( $plugin_file != $plugin_data['Name'] )
				$plugin_app .= '<br/>' . $plugin_data['Name'];
		...
		if ( 'dropins' != $context ) {
			...
			$plugin_app = $plugin_data['Name'];
		}

		if ( ! empty( $totals['upgrade'] ) && ! empty( $plugin_data['update'] ) )
			$class .= ' update';

		$plugin_slug = isset( $plugin_data['slug'] ) ? $plugin_data['slug'] : sanitize_title( $plugin_app );
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				case 'cb':
					...
					break;
				case 'name':
					echo "<td class='plugin-title column-primary'><strong>$plugin_app</strong>";
				...
				case 'description':
					...
					if ( isset( $plugin_data['slug'] ) && current_user_can( 'install_plugins' ) ) {
						$plugin_meta[] = sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
							...
							esc_attr( sprintf( __( 'More information about %s' ), $plugin_app ) ),
							esc_attr( $plugin_app ),
							__( 'View details' )
						);
