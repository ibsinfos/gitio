<?php
$output = $type = $shape = $link = $image_url = $image_id = $title = $sub_title = $view_type = $date = $img_width = $align = $hover_bg = $hover_img = $centered_icons = $icons = $noborders = $show_socials = $socials = $animation_type = $animation_duration = $animation_delay = $el_class = '';
...
if ($image_url) {
    ...
    if ($type == '') {
        ...
        if ($shape !== 'thumbnail') {
            $output .= '<img alt="" src="' . $image_url . '" class="img-responsive img-' . $shape . '">';
        } else {
            $output .= '<span class="img-thumbnail">';
            $output .= '<img alt="" src="' . $image_url . '" class="img-responsive">';
            $output .= '</span>';
        }
        ...
        $output .= '<span class="thumb-info' . ($align ? ' align-' . $align : '') . ($hover_bg ? ' thumb-info-' . $hover_bg : '') . ($hover_img ? ' thumb-info-' . $hover_img : '') . ($centered_icons ? ' thumb-info-centered-icons' : '') . ($view_class ? ' ' . $view_class : '') . ($noborders ? ' thumb-info-no-borders' : '') . '">';
