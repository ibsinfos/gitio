...
?><article id="post-<?php the_ID(); ?>"
	...
	if ( !in_array($post_format, array('link', 'aside', 'status', 'quote')) ) {
		...
		<div class="post_header entry-header">
			...
			if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
				...
				?><span class="post_date"><i class="demo-icon icon-clock-empty"></i> <?php echo get_the_date(); ?></span></div><?php
			}
			?>
		</div><!-- .entry-header -->
		<?php
	...
	if ($number <= $featured) {
		...
		<div class="post_footer entry-footer">
			...
			?> -->
			<a href='<?php echo get_permalink(); ?>' class="sc_action_item_link sc_button_hover_slide_left sc_button_hover_style_hover">learn more</a>
		</div><!-- .entry-footer -->
		<?php
	}
?>
