...
class trx_addons_widget_recent_news extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'translang'));
		parent::__construct( 'trx_addons_widget_recent_news', esc_html__('ThemeREX Addons - Recent News', 'translang'), $widget_ops );
	}
	...
	function form($instance) {
		...
		$list_styles = array(
			'news-announce'	=> esc_html__('Announce',	'translang'),
			'news-excerpt'	=> esc_html__('Excerpt',	'translang'),
			'news-magazine'	=> esc_html__('Magazine',	'translang'),
			'news-portfolio'=> esc_html__('Portfolio',	'translang')
		);

		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('widget_title')); ?>"><?php esc_html_e('Widget title:', 'translang'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Block title:', 'translang'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('subtitle')); ?>"><?php esc_html_e('Block subtitle:', 'translang'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('style')); ?>"><?php esc_html_e('Style:', 'translang'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed:', 'translang'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('featured')); ?>"><?php esc_html_e('Number of featured posts:', 'translang'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('columns')); ?>"><?php esc_html_e('Number of columns:', 'translang'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('category')); ?>"><?php esc_html_e('Parent category:', 'translang'); ?></label>
			<?php
			wp_dropdown_categories(array(
				...
				'selected' => $category,
				'hierarchical' => true,
				'show_option_none' => esc_html__('-- All categories --', 'translang')
				)
			);
			?>
		</p>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show categories:', 'translang'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_1"><?php esc_html_e('Show', 'translang'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_categories')); ?>_0"><?php esc_html_e('Hide', 'translang'); ?></label>
		</p>
			...
			$output	.= '<div class="sc_recent_news_header'.(trx_addons_is_on($show_categories) && !empty($cats) ? ' sc_recent_news_header_split' : '').'">'
							. ( !empty($title) || !empty($subtitle)
								? '<div class="sc_recent_news_header_captions">'
										. (!empty($subtitle) ? '<h6 class="sc_item_subtitle sc_recent_news_subtitle">' . esc_html($subtitle) . '</h6>' : '')
										. (!empty($title) ? '<h2 class="sc_item_title sc_item_title_style_default sc_recent_news_title">' . esc_html($title) . '</h2>' : '')
									. '</div>'
								: '');
			...
			if (trx_addons_is_on($show_categories) && !empty($cats)) {
				...
				if (is_array($cats) && count($cats) > 0) {
					$output .= '<a href="' . esc_url( $category == 0
						...
						: get_category_link($category) ) . '" class="sc_recent_news_header_category_item">'.esc_html__('All News', 'translang').'</span>';
					$number = 0;
					$number_max = 3;
					foreach ($cats as $cat) {
						$number++;
						if ($number == $number_max)
							$output .= '<span class="sc_recent_news_header_category_item sc_recent_news_header_category_item_more">'.esc_html__('More', 'translang')
...
if (!function_exists('trx_addons_sc_recent_news_add_in_vc_params')) {
	function trx_addons_sc_recent_news_add_in_vc_params() {
		...
		$list_sort = array(
			...
			"ID" 		=> esc_html__('Post ID', 'translang'),
			"date"		=> esc_html__("Date", 'translang'),
			"title"		=> esc_html__("Alphabetically", 'translang'),
			"views"		=> esc_html__("Popular (views count)", 'translang'),
			"comments"	=> esc_html__("Most commented (comments count)", 'translang'),
			"random"	=> esc_html__("Random", 'translang')
		);
		$list_order = array(
			"asc"  => esc_html__("Ascending", 'translang'),
			"desc" => esc_html__("Descending", 'translang')
		);

		return apply_filters('trx_addons_sc_map', array(
				"base" => "trx_widget_recent_news",
				"name" => esc_html__("Recent News", 'translang'),
				"description" => wp_kses_data( __("Insert recent news list", 'translang') ),
				"category" => esc_html__('ThemeREX', 'translang'),
				...
				"params" => array_merge(
					array(
						array(
							...
							"heading" => esc_html__("Widget Title", 'translang'),
							"description" => wp_kses_data( __("Title for the widget (fill this field only if you want to use shortcode as widget)", 'translang') ),
						...
						array(
							"param_name" => "title",
							"heading" => esc_html__("Title", 'translang'),
							"description" => wp_kses_data( __("Title for the block", 'translang') ),
							'edit_field_class' => 'vc_col-sm-4',
							"admin_label" => true,
							"type" => "textfield"
						),
						array(
							"param_name" => "subtitle",
							"heading" => esc_html__("Subtitle", 'translang'),
							"description" => wp_kses_data( __("Subtitle for the block", 'translang') ),
							'edit_field_class' => 'vc_col-sm-4',
							"type" => "textfield"
						),
						array(
							"param_name" => "style",
							"heading" => esc_html__("List style", 'translang'),
							"description" => wp_kses_data( __("Select style to display news list", 'translang') ),
							"admin_label" => true,
						...
						array(
							...
							"heading" => esc_html__("Show categories", 'translang'),
							"description" => wp_kses_data( __("Show categories in the shortcode's header", 'translang') ),
							"std" => "0",
							'edit_field_class' => 'vc_col-sm-6',
						...
						array(
							"param_name" => "ids",
							"heading" => esc_html__("List IDs", 'translang'),
							"description" => wp_kses_data( __("Comma separated IDs list to show. If not empty - parameters 'cat', 'offset' and 'count' are ignored!", 'translang') ),
							"group" => esc_html__('Query', 'translang'),
							"type" => "textfield"
						),
						array(
							"param_name" => "category",
							"heading" => esc_html__("Category", 'translang'),
							"description" => wp_kses_data( __("Select category to show news. If empty - select news from any category or from IDs list", 'translang') ),
							"group" => esc_html__('Query', 'translang'),
							'edit_field_class' => 'vc_col-sm-4',
							'dependency' => array(
								'element' => 'ids',
							...
							"std" => 0,
							"value" => array_flip(trx_addons_array_merge(array(0 => esc_html__('- Select category -', 'translang')), trx_addons_get_list_categories())),
							"type" => "dropdown"
						),
						array(
							"param_name" => "count",
							"heading" => esc_html__("Total posts", 'translang'),
							"description" => wp_kses_data( __("How many posts will be displayed? If used IDs - this parameter ignored.", 'translang') ),
							"admin_label" => true,
							'edit_field_class' => 'vc_col-sm-4',
							'dependency' => array(
								'element' => 'ids',
						...
						array(
							"param_name" => "columns",
							"heading" => esc_html__("Columns", 'translang'),
							"description" => wp_kses_data( __("How many columns use to show news list", 'translang') ),
							"group" => esc_html__('Query', 'translang'),
							"admin_label" => true,
							'edit_field_class' => 'vc_col-sm-4',
							'dependency' => array(
						...
						array(
							"param_name" => "offset",
							"heading" => esc_html__("Offset before select posts", 'translang'),
							"description" => wp_kses_data( __("Skip posts before select next part.", 'translang') ),
						...
						array(
							"param_name" => "featured",
							"heading" => esc_html__("Featured posts", 'translang'),
							"description" => wp_kses_data( __("How many posts will be displayed as featured?", 'translang') ),
							"admin_label" => true,
							"group" => esc_html__('Query', 'translang'),
							'edit_field_class' => 'vc_col-sm-6',
							'dependency' => array(
						...
						array(
							"param_name" => "orderby",
							"heading" => esc_html__("Post sorting", 'translang'),
							"description" => wp_kses_data( __("Select desired posts sorting method", 'translang') ),
							"group" => esc_html__('Query', 'translang'),
							'edit_field_class' => 'vc_col-sm-6 vc_new_row',
						...
						array(
							"param_name" => "order",
							"heading" => esc_html__("Post order", 'translang'),
							"description" => wp_kses_data( __("Select desired posts order", 'translang') ),
							"group" => esc_html__('Query', 'translang'),
							'edit_field_class' => 'vc_col-sm-6',
