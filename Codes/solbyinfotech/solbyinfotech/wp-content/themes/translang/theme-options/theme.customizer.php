...
if ( !function_exists('translang_init_meta_box') ) {
	add_action( 'after_setup_theme', 'translang_init_meta_box' );
	function translang_init_meta_box() {
		if ( is_admin() ) {
			add_action("admin_enqueue_scripts", 'translang_add_meta_box_scripts');
			add_action('save_post',			'translang_save_meta_box');
...
if ( !function_exists( 'translang_add_meta_box_scripts' ) ) {
	add_action("admin_enqueue_scripts", 'translang_add_meta_box_scripts');
	function translang_add_meta_box_scripts() {
		// If current screen is 'Edit Page' - load fontello
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		if (is_object($screen) && translang_allow_meta_box(!empty($screen->post_type) ? $screen->post_type : $screen->id)) {
...
if (!function_exists('translang_allow_meta_box')) {
	function translang_allow_meta_box($post_type) {
		return apply_filters('translang_filter_allow_meta_box', in_array($post_type, array('page', 'post')), $post_type);
	}
}

// Add meta box
if (!function_exists('translang_add_meta_box')) {
	//Handler of the add_action('add_meta_boxes', 'translang_add_meta_box');
	function translang_add_meta_box() {
		global $post_type;
		if (translang_allow_meta_box($post_type)) {
			add_meta_box(sprintf('translang_meta_box_%s', $post_type),
						esc_html__('Theme Options', 'translang'),
						'translang_show_meta_box',
						$post_type,
...
if (!function_exists('translang_show_meta_box')) {
	function translang_show_meta_box() {
		...
		if (translang_allow_meta_box($post_type)) {
			...
			foreach ($TRANSLANG_STORAGE['options'] as $k=>$v) {
				...
				$v['val'] = isset($meta[$k]) ? $meta[$k] : 'inherit';
				$tabs_content[$v['override']['section']] .= translang_show_meta_box_field($k, $v);
			}
...
if ( !function_exists('translang_show_meta_box_field') ) {
	function translang_show_meta_box_field($name, $field) {
...
if (!function_exists('translang_save_meta_box')) {
	//Handler of the add_action('save_post', 'translang_save_meta_box');
	function translang_save_meta_box($post_id) {
